...
watch(search, (value) => {
    Form.search = value
    Form.get(route('JadwalHarian.index'), {
        preserveScroll: true,
        preserveState: true,
    })
})
...
function deleteItem() {
    DeleteForm.delete(route('JadwalHarian.destroy'), {
        preserveState: true,
        preserveScroll: true,
        onFinish: () => {
...
<template>
    ...
    <Head title="Jadwal Harian" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-100 leading-tight">Data Jadwal Harian</h2>
        </template>
            ...
            <div class="border divide-y divide-gray-200 max-w-7xl mb-8 overflow-hidden rounded-lg shadow-xs bg-white">
                <div class="py-3 px-4">
                    <div class="relative max-w-xs">
                        <Link :href="route('JadwalHarian.create')">
                                        ...
                                        <dropdownTable align="top" width="48" :key="index">
                                            ...
                                            <template #content>
                                                ...
                                                <DropdownLink v-if="crud.edit"
                                                    :href="route('JadwalHarian.edit', { slug: item.id })"
                                                ...
                                                <DropdownLink v-if="crud.show"
                                                    :href="route('JadwalHarian.show', { slug: item.id })"
                                                    class="flex justify-start gap-3 text-gray-700">
                                                    <font-awesome-icon class="text-blue-500 hover:text-blue-700"
                                                        :icon="['fas', 'eye']" />
                                                    Detail
                                                ...
                                                <DropdownLink v-if="crud.reset_password"
                                                    :href="route('JadwalHarian.reset.password', { slug: item.user.id })"
                                                    class="flex justify-start gap-3 text-gray-700">
                                                    <font-awesome-icon class="text-blue-500 hover:text-blue-700"
                                                        :icon="['fas', 'key']" />
                                                    Reset Password
